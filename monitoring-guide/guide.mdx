...
flowchart TB
    ...
    subgraph connected["Connected Cluster"]
        ...
        ConnOTel["OTel Collector\n(Deployment + Target Allocator)"]

        subgraph connPrivVC["Virtual Cluster (Private)"]
            ConnPrivOTel["OTel Collector\n(DaemonSet)"]
        end
    end
...
<Flow id="deploy-otel-private">
  ...
  <Step>
    Click on the virtual cluster, then navigate to the <NavStep>Apps</NavStep>
    tab.
  </Step>
  <Step>
    Click <Button>Deploy App</Button> and select the
...
2. Click **New** > **Import**.
3. Paste the JSON content of the dashboard file, or upload the JSON file
   directly.
...
**File:** `dashboards/vcluster-projects.json`
...
| Section | Panels |
|---------|--------|
| Project summary | Total projects, total vClusters, total pods, total CPU (cores), total memory, max error rate |
| Resource usage by project | CPU by project, memory by project, pods by project, vClusters by project (bar charts) |
| API health by project | API request rate, API error rate (5xx), API P95 latency (time series) |
| Resource trends | CPU usage trend (stacked), memory usage trend (stacked) |

<!-- vale off -->
### vCluster detail dashboard
<!-- vale on -->

**File:** `dashboards/vcluster-detail.json`
...
| Section | Panels |
|---------|--------|
| vCluster overview | Pods, containers, CPU (cores), memory, error rate, API request rate (stat panels) |
| Workload resources | CPU by namespace, memory by namespace (time series), top pods by CPU, top pods by memory (tables) |
| vCluster control plane | API request rate, API error rate (5xx), API P95 latency, inflight requests (time series) |
| Container lifecycle | Container restarts, OOM kills |
